...
object MaxOfSlidingWindow {
    ...
    fun computeTrafficVolumes(A: List<TrafficElement>, w: Int): List<TrafficElement> {
        val slidingWindow: QueueWithMaxUsingDeque.QueueWithMax<TrafficElement> = QueueWithMax()
        val maximumVolumes: List<TrafficElement> = ArrayList()
        for (trafficInfo in A) {
            slidingWindow.enqueue(trafficInfo)
            while (trafficInfo.time - slidingWindow.head().time > w) {
                slidingWindow.dequeue()
            }
            maximumVolumes.add(
                    TrafficElement(trafficInfo.time, slidingWindow.max().volume))
        }
