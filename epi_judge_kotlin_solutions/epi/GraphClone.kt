...
object GraphClone {
    fun cloneGraph(graph: GraphVertex?): GraphVertex? {
        ...
        val vertexMap: Map<GraphVertex, GraphVertex> = HashMap()
        val q: Queue<GraphVertex> = ArrayDeque()
        q.add(graph)
        vertexMap.put(graph, GraphVertex(graph.label))
        while (!q.isEmpty()) {
            val v: GraphVertex = q.remove()
            for (e in v.edges) {
                // Try to copy vertex e.
                if (vertexMap.putIfAbsent(e, GraphVertex(e.label)) == null) {
                    q.add(e)
                }
                // Copy edge.
                vertexMap[v]!!.edges.add(vertexMap[e])
            }
        }
        return vertexMap[graph]
    }
